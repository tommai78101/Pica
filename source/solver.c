...
void Solver_Init(C3D_ContactSolver* solver, C3D_Island* island)
{
	...
	solver->contactConstraintCount = island->contactConstraintCount;
...
{
	for (unsigned int i = 0; i < solver->contactConstraintCount; i++)
	{
...
void Solver_PreSolve(C3D_ContactSolver* solver, float deltaTime)
{
	for (unsigned int i = 0; i < solver->contactConstraintCount; i++)
	{
		...
		for (int j = 0; j < constraintState->contactCount; j++)
		{
			...
			float netMass = constraintState->inverseMassA + constraintState->inverseMassB;
			float totalMass[2];
			totalMass[0] = netMass;
			totalMass[1] = netMass;
			netMass += FVec3_Dot(contactPointA_Cross_normal, Mtx_MultiplyFVec3(&constraintState->inertiaA, contactPointA_Cross_normal)) +
			           FVec3_Dot(contactPointB_Cross_normal, Mtx_MultiplyFVec3(&constraintState->inertiaB, contactPointB_Cross_normal));
			...
			for (int k = 0; k < 2; k++)
			{
				...
				totalMass[k] += FVec3_Dot(contactPointA_Cross_Tangent, Mtx_MultiplyFVec3(&constraintState->inertiaA, contactPointA_Cross_Tangent)) +
				                FVec3_Dot(contactPointB_Cross_Tangent, Mtx_MultiplyFVec3(&constraintState->inertiaB, contactPointB_Cross_Tangent));
			...
			vAxisA = FVec3_Subtract(vAxisA, FVec3_Scale(warmStartContact, constraintState->inverseMassA));
			wAxisA = FVec3_Subtract(wAxisA, Mtx_MultiplyFVec3(&constraintState->inertiaA, FVec3_Cross(contactState->radiusContactA, warmStartContact)));
			vAxisB = FVec3_Add(vAxisB, FVec3_Scale(warmStartContact, constraintState->inverseMassB));
			wAxisB = FVec3_Add(wAxisB, Mtx_MultiplyFVec3(&constraintState->inertiaB, FVec3_Cross(contactState->radiusContactB, warmStartContact)));
		...
		for (int j = 0; j < constraintState->contactCount; j++)
		{
			...
			if (solver->enableFriction)
			{
				for (int k = 0; k < 2; k++)
				{
					...
					vAxisA = FVec3_Subtract(vAxisA, FVec3_Scale(impulse, constraintState->inverseMassA));
					wAxisA = FVec3_Subtract(wAxisA, Mtx_MultiplyFVec3(&constraintState->inertiaA, FVec3_Cross(contactState->radiusContactA, impulse)));
					vAxisB = FVec3_Add(vAxisB, FVec3_Scale(impulse, constraintState->inverseMassB));
					wAxisB = FVec3_Add(wAxisB, Mtx_MultiplyFVec3(&constraintState->inertiaB, FVec3_Cross(contactState->radiusContactB, impulse)));
				}
			}
